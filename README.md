...
### RTK (Rust Token Killer) 統合

このリポジトリには、LLM トークン消費を 60-90% 削減する [RTK](https://github.com/rtk-ai/rtk) が統合されています。

#### RTK とは

RTK は、git、test、lint などの開発コマンドの出力をフィルタリング・圧縮することで、AI エージェントのトークン消費を大幅に削減する CLI プロキシツールです。

#### 自動更新

- `update-ai-agents.sh` スクリプトに RTK の自動更新機能が統合されています
- Homebrew または cargo でインストールされた RTK を自動的に更新します
- 各 AI CLI（claude, gemini など）の実行時に自動的にチェックされます

#### エイリアス設定

bash と zsh の両方で rtk ラッパー関数が設定されており、以下のように動作します:

```bash
# rtk コマンドを実行すると自動的に更新チェックが行われる
rtk gain              # トークン節約統計を表示
rtk git status        # Git コマンドを RTK 経由で実行
```

#### Claude Code 統合

Claude Code には RTK awareness が設定されており、`@RTK.md` を参照することで RTK の使用方法を理解できます。

#### 設定ファイル

RTK の設定ファイルは `~/.config/rtk/config.toml` として chezmoi で管理されています。
