...
1. **Worktree の作成**: `EnterWorktree` でこのチケット専用の作業ツリーを作成
2. **Jira チケット情報の取得**: Jira MCP でチケット内容を取得
3. **spec（要件定義書）の作成**: `superpowers:brainstorming` により要件を対話的に確認し、spec を作成
4. **spec のレビュー・Confluence アップロード・承認**: sub-agent レビュー後 Confluence にアップロードし、`AskUserQuestion` で承認を得る
5. **plan（実装計画）の作成・レビュー・Confluence アップロード・承認**: spec と同様の流れを plan にも適用
6. **チケットへのコメント投稿**: spec/plan の Confluence URL をチケットにコメント
7. **ブランチ作成・実装**: チケットタイプからブランチタイプを決定してブランチを作成し、plan を実行（PR に Jira への言及は含めない）
8. **検証・ローカルコードレビュー**: 動作確認と `/deep-review` を実施
9. **PR 作成**: PR を作成
10. **チケットの更新**: PR URL を含む完了コメントを投稿
11. **ポスト PR フロー**: `/pr-health-monitor` で CI 確認・Copilot レビュー対応を実行
