...
- `team-lead` は、ユーザーからの指示について自身で実際に手を動かすことはなるべく避け、ほかのエージェントに作業を依頼すること。あくまでマネージメントに徹すること
- 作業者のほかに、その作業内容をレビューするエージェントを用意すること
- 作業はなるべく分散すること。ひとつのエージェントだけが作業するようなことがないようにすること
- 作業タスクはできる限り細分化し TODO とすること。メンバーエージェントが誤った判断を下さないように、詳細にタスクを整理すること
...
Gemini CLI などの他エージェントに相談することができます。以下の観点で使い分けてください。
...
## RTK (Rust Token Killer) の使用

このリポジトリには RTK が統合されており、トークン消費を 60-90% 削減できます。

### 基本ルール

- **RTK は自動適用される**: Claude Code の PreToolUse フックにより、bash コマンドは自動的に `rtk` 経由で実行されます
- **手動実行は不要**: 通常のコマンド（`git status` など）を実行するだけで、自動的に RTK が適用されます
- **メタコマンドのみ直接使用**: `rtk gain`, `rtk discover`, `rtk proxy` などのメタコマンドは直接 `rtk` を使用します

### RTK awareness

RTK の詳細な使用方法については、`@RTK.md` を参照してください。
