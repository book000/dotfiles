...
Claude Code では Agent Teams というマルチセッションでの並行開発が可能です。この機能は積極的に使用する必要があります。

- 調査・開発の作業においては必ず使用する
- チームは、「マネージャー」・「リーダー」・「メンバー」の3つの役割を持ったセッション群で構成する
- リーダーとメンバーは必須
- メンバーは複数人いても良い
- リーダーは、メンバーに対してタスクを割り当て、管理する役割を担う。リーダーが手を動かしてはならない
- 大きなタスクの場合は、マネージャーがリーダーを監視し、リーダーを補助する
...
- `UserPromptSubmit` フックが毎回 inbox をスキャンし、受信メッセージをプロンプトに自動注入する
...
プロンプトの末尾に `[tmux IPC]` セクションが付加されていたら、内容を確認して必要に応じて対応すること。
