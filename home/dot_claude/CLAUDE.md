...
5. GitHub Copilot からのレビューコメントを Agent を用いてバックグラウンドで待機する
7. レビューコメントへの対応を行うこと。**レビューコメント対応漏れを防ぐため、以下を必ず順序通りに実施すること:**
...
8. `/code-review:code-review` によるコードレビューを実施し、**スコア 50 以上の指摘事項に必ず対応すること**
