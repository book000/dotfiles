...
**chezmoi 管理の dotfiles について**: chezmoi には独自のファイル命名規則があり、変更を反映するには `chezmoi apply` が必要。標準のファイルパスが機能すると仮定しない。**編集前に chezmoi のソース状態を確認すること**。
...
**重要**: PR 作成後、**無限のレビュー/CI 待機ループに入らない**こと。1 回のコードレビューパスを実行し、必要に応じて修正をプッシュし、その後ユーザーにステータスを報告する。**明示的に要求されない限り、外部レビュー（Copilot、CI）を繰り返しポーリングしない**。
