...
Claude Code では Agent Teams というマルチセッションでの並行開発が可能です。この機能は積極的に使用する必要があります。調査・開発の作業においては必ず使用すること。
...
- `PostToolUse` フックがツール実行後に inbox をスキャンし、受信メッセージをツール出力に追記する
...
ツール出力の末尾に `[tmux IPC]` セクションが付加されていたら、内容を確認して必要に応じて対応すること。
