...
#     less /tmp/install.sh  # Verify the script
#     bash /tmp/install.sh
#
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 ]]; then
    log_error "インストールがエラーで終了しました (exit code: $exit_code)"
    log_error "上記のログを確認してください"
  fi
}
trap cleanup EXIT
      ...
      if [[ -f /etc/os-release ]]; then
        ...
        case "$DISTRO" in
          ubuntu|debian)
            ...
            ;;
          *)
            log_error "サポートされていないディストリビューション: $DISTRO"
            log_error "このスクリプトは Ubuntu と Debian のみをサポートしています"
            return 1
            ;;
        esac
      else
        log_error "Linux ディストリビューションを検出できません"
        return 1
      fi
      ;;
    darwin*)
      log_error "macOS はまだサポートされていません"
      return 1
      ;;
    msys*|cygwin*)
      log_error "Windows はまだサポートされていません"
      return 1
      ;;
    *)
      log_error "サポートされていない OS: $OSTYPE"
      return 1
      ;;
  esac
