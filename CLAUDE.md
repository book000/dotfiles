...
`home/dot_claude/skills/check-container-status/` は、多数の Docker Compose
プロジェクトが同居するディレクトリ(例: `/mnt/hdd/<マシン名>`)の稼働状況を、
並列サブエージェント(`home/dot_claude/agents/container-status-checker.md` /
`container-error-investigator.md`)で網羅的に確認するグローバルスキル。
